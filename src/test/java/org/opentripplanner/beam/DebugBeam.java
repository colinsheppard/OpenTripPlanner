...
package org.opentripplanner.beam;
...
public class DebugBeam {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(DebugBeam.class);
    ...
    public static void main(String[] args) {
        ...
        DebugBeam main = new DebugBeam(params);
        main.run();

    }

    /* Constructor. */
    public DebugBeam(CommandLineParameters params) {
        this.params = params;
    }
    ...
    public void run() {
        ...
        request.routerId = "berkeley";
        request.clearModes();
        request.addMode(TraverseMode.WALK);
        request.addMode(TraverseMode.TRANSIT);
        request.addMode(TraverseMode.BUS);
        request.from = new GenericLocation( 37.846351, -122.282965);
        request.to = new GenericLocation(37.891427, -122.278185);
        request.dateTime = ZonedDateTime.parse("2016-10-17T06:58:00-07:00[UTC-07:00]").toEpochSecond();
        request.maxWalkDistance = 804.672;
        request.locale = Locale.ENGLISH;
        GraphPathFinder gpFinder = new GraphPathFinder(graphService.getRouter());
